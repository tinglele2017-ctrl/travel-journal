    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="flex gap-1 rounded-lg bg-gray-100 p-1">
          {["全部", "游记", "目的地"].map((f) => (
            <button key={f} className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${f === "全部" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>{f}</button>
          ))}
        </div>
      </div>

      {q && <div className="mb-4 text-sm text-gray-500">找到 <span className="font-medium text-gray-900">{results.length}</span> 条结果{q && <span>，关键词：「{q}」</span>}</div>}

      {q && results.length === 0 && <div className="py-20 text-center"><div className="mb-4 text-5xl">🔍</div><p className="text-gray-500">没有找到「{q}」相关内容</p></div>}
          ...
          <Link key={j.id} href={`/journal/${j.id}`} className="group flex gap-4 rounded-xl border bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-4 text-sm text-gray-400">
                <span>{j.author.username}</span><span>{j.totalDays} 天</span><span>❤ {j.likeCount}</span><span>⭐ {j.collectCount}</span>
              </div>
            </div>
          </Link>
        ))}
